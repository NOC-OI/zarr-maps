 ...
 * used by web-map visualization components.
 *
 * Provides:
...
function getMultiscaleLevelPaths(
  ...
  if (multiscaleFormat === 'geozarr') return geozarrLevelPaths;
...
export async function loadDimensionValues(
  ...
  const cached = dimensionValues[dimIndices.name];
  // Cached values have no level identity. They are safe for single-scale/root
  // coordinates, but a multiscale level must resolve its own coordinate array.
  if (cached && !levelInfo) {
    return slice ? cached.slice(slice[0], slice[1]) : cached;
  }
  ...
  let coordArr: zarr.Array<any> | null = null;
  if (levelInfo) {
    try {
      coordArr = await localFunc(targetRoot.resolve(dimIndices.name), { kind: 'array' });
    } catch {
      // Some pyramids keep shared coordinates at the dataset root. Fall back
      // to the array discovered during metadata initialization in that case.
    }
  }
  if (!coordArr && dimIndices.array) {
    coordArr = dimIndices.array;
  }
  if (!coordArr) {
    coordArr = await localFunc(root.resolve(dimIndices.name), { kind: 'array' });
  }
  if (!coordArr) {
    throw new Error(`Unable to open coordinate array ${dimIndices.name}`);
  }
  const resolvedCoordArr = coordArr as zarr.Array<any>;
  const coordData = await zarr.get(resolvedCoordArr);
  ...
  if (dimIndices.name === 'time') {
    try {
      const units = resolvedCoordArr.attrs.units as string;
      const calendar = resolvedCoordArr.attrs.calendar as CFCalendar | undefined;
...
export async function initZarrDataset(
  ...
  if (levelPaths.length > 0) {
    pyramidMode = true;
    levelInfos = [...levelPaths];

    // Open the requested cube level first so its coordinate arrays are the ones
    // attached to dimIndices. Continue through the remaining levels to retain
    // complete, correctly-indexed metadata for zoomable 2D providers.
    const selectedIndex = multiscaleLevel ?? 0;
    const levelIndices = [
      selectedIndex,
      ...levelPaths.map((_, index) => index).filter(index => index !== selectedIndex)
    ].filter(index => index >= 0 && index < levelPaths.length);
    for (const i of levelIndices) {
...
async function calculateCoordinatesFromAttrs(
  ...
  // Zarr v3 stores dimension names in array metadata. Zarrita exposes those
  // through `dimensionNames`; `_ARRAY_DIMENSIONS` is the conventional v2
  // location and remains the fallback for older datasets.
  let array_dimensions: string[] =
    arr.dimensionNames ?? (arr.attrs['_ARRAY_DIMENSIONS'] as string[]) ?? [];
  if (array_dimensions.length === 0) {
    const location = zarr.root(store);
    const metadataBytes = await store.get(location.resolve('zarr.json').path);
    if (metadataBytes) {
      const rootMetadata = JSON.parse(new TextDecoder().decode(metadataBytes));
      const consolidated = rootMetadata.consolidated_metadata?.metadata;
      if (consolidated) {
        const arrayPath = arr.path.replace(/^\//, '');
        const arrayMetadata =
          consolidated[arrayPath] ??
          consolidated[variable] ??
          Object.entries(consolidated).find(([path]) => path.endsWith(`/${variable}`))?.[1];
        array_dimensions = (arrayMetadata as { dimension_names?: string[] } | undefined)
          ?.dimension_names ?? [];
      }
    }
  }
  if (array_dimensions.length === 0) {
    throw new Error(
      `Unable to determine dimensions for Zarr array ${arr.path}. ` +
        'Expected Zarr v3 dimension_names or the v2 _ARRAY_DIMENSIONS attribute.'
    );
  }
 ...
 */
export async function getZarrData(
  arr: zarr.Array<any>,
  selection?: any,
  options?: any
): Promise<any> {
  return zarr.get(arr, selection, options);
}
